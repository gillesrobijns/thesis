...
for i in range(N_TEST_IMG):
    ...
    psnr                    = compare_psnr(test_data_full[i],denoised_sdA3)
    sim                     = ssim(test_data_full[i],denoised_sdA3)
    ...
    if i < N_VIEW_IMG:
        ...
        a[1][i].axis('off')
        a[2][i].imshow(denoised_sdA3, cmap='gray')
        a[2][i].axis('off')
