...
class DVarAutoEncoder(nn.Module):
    
    def __init__(self,inputSize,midSize,muSize,varSize):
        super(DVarAutoEncoder, self).__init__()
...
DVAE            = DVarAutoEncoder(patch_size*patch_size,HUNITS,HUNITS_mu,HUNITS_var)

optimizer       = torch.optim.Adam(DVAE.parameters(), lr=LR)
...
def train(epoch):
    ...
    for batch_idx, data in enumerate(train_loader):
        ...
        data_noise  = noise(data.data,MEAN,STDDEV)
        data_noise  = Variable(data_noise)
        
        optimizer.zero_grad()
        
        recon_batch, mu, logvar     = DVAE(data_noise)
...
def test(epoch):
    ...
    for i, data in enumerate(test_loader):
        ...
        recon_batch, mu, logvar     = DVAE(data)
...
plt.suptitle('Denoising Variational Autoencoder', fontsize=16)
...
decoded_data, mu, logvar    = DVAE(view_noise_data)
