...
def DVAE(data,inputSize,midSize,muSize,varSize):
    
        t0                      = time()
    
        DVAE                    = DVarAutoEncoder(inputSize,midSize,muSize,varSize)
        data                    = torch.from_numpy(data).float()
        data                    = Variable(data.view(-1, inputSize))
        
        DVAE.load_state_dict(torch.load('DVAE.pt'))
        
        decoded,_,_             = DVAE(data)
        
        dt                      = time() - t0
        
        print('Denoising Variational Autoencoder done in %.2fs.' % dt)
