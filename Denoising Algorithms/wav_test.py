...
print('Starting Wavelet Denoising')
...
plt.suptitle('Wavelet thresholding', fontsize=16)
...
for i in range(N_TEST_IMG):
    ...
    denoised_wavelet        = denoise_wavelet(test_data_full_noise[i], sigma=0.4,wavelet='db1')
    
    dt                      = time() - t0
    
    psnr                    = compare_psnr(test_data_full[i],denoised_wavelet)
    sim                     = ssim(test_data_full[i],denoised_wavelet)
    ...
    if i < N_VIEW_IMG:
        ...
        a[1][i].axis('off')
        a[2][i].imshow(denoised_wavelet, cmap='gray')
        a[2][i].axis('off')
